...
// OFFER PAGE
const addoffer_page = async (req, res) => {
    try {
        const catagory = await categorydb.find({})
        const brand = await branddb.find({})
        // const coupons = await coupondb.find({})
        const product = await productdb.find({})
        res.render('addoffer', { category: catagory, brand: brand,  product: product })
    } catch (error) {
        console.log(error.message);
    }
}
// POST - ADDOFFER PRODUCT
const post_addoffer  =  async(req,res)=>{
    const arrimages = []
    if (req.files) {
        for (let i = 0; i < req.files.length; i++) {
            arrimages.push(req.files[i].filename)
    ...
    const offer_price = req.body.price * req.body.offer / 100
    const offer_amt = req.body.price - offer_price
    // console.log(offer_amt)
    ...
    try {
       ...
       const offer = new offerdb({
        ...
        quantity: req.body.quantity,
        color:req.body.color,
        offerpercentage:req.body.offer,
        image:arrimages,
        category:req.body.category,
        category1:req.body.category1,
        size:req.body.size,
        expiry:req.body.date,
        offerprice: offer_amt
       })
       await offer.save()
       res.redirect('/admin/offerproducts')
...
// RENDERING OFFER PRODUCT PAGE
const page_offer = async(req,res)=>{
    try {
        const offerlist = await offerdb.find({})
        res.render('offer',{offerlist:offerlist})
    } catch (error) {
        console.log(error)
    }
}
...
const return_apporved = async (req, res) => {
    try {
